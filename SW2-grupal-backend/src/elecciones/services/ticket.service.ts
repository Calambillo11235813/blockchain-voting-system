...
export class TicketService {
     ...
     */
    async create(createTicketDto: CreateTicketDto & { section?: Section }, userId: string): Promise<ApiResponse<Ticket>> {
        try {
     ...
     */
    async createFromSection(sectionId: string, userId: string): Promise<ApiResponse<Ticket>> {
        const createTicketDto = new CreateTicketDto();
        createTicketDto.sectionId = sectionId;
        createTicketDto.date = new Date();
        return this.create(createTicketDto, userId);
    }

    /**
     * Obtener un ticket por ID
     */
    async findOne(id: string, userId: string): Promise<ApiResponse<Ticket>> {
        try {
     ...
     */
    async findAll(userId: string, paginationDto: PaginationDto): Promise<ApiResponse<Ticket[]>> {
        try {
            const {
                limit = 10,
                offset = 0,
            ...
            const queryBuilder = this.ticketRepository.createQueryBuilder('ticket')
                ...
                .where('ticket.is_active = :isActive', { isActive: true });
     ...
     */
    async findBySection(sectionId: string, userId: string, paginationDto: PaginationDto): Promise<ApiResponse<Ticket[]>> {
        try {
            const {
                limit = 10,
                offset = 0,
     ...
     */
    async patch(id: string, updateTicketDto: UpdateTicketDto & { section?: Section }, userId: string): Promise<ApiResponse<Ticket>> {
        try {
            const ticket = await this.ticketRepository.findOne({
                where: {
                    id,
     ...
     */
    async getTicketsBySection(sectionId: string, userId: string): Promise<ApiResponse<Ticket[]>> {
        try {
            // Verificar que la sección existe
            const section = await this.sectionRepository.findOne({
                where: {
    ...
    async validateTicket(
        ticketData: ValidateTicketDto,
        userId: string
    ): Promise<ApiResponse<any>> {
        try {
            console.log('Datos recibidos:', ticketData, userId);

            if (!ticketData.ticketId) {
                return createApiResponse(HttpStatus.BAD_REQUEST, {
                    isValid: false,
                    message: 'Datos inválidos: ticketId es requerido'
                }, 'Validación de ticket completada');
            }

            const ticket = await this.ticketRepository.findOne({
                where: { id: ticketData.ticketId, is_active: true },
                relations: ['section', 'section.event']
            });
            ...
            if (ticketData.ticketSection && ticket.section.name !== ticketData.ticketSection) {
                return createApiResponse(HttpStatus.OK, {
                    isValid: false,
                    message: `La sección en el ticket (${ticketData.ticketSection}) no coincide con la registrada (${ticket.section.name})`
                }, 'Validación de ticket completada');
            }
            ...
            try {
                ...
                const blockchainResult = await this.ticketValidatorContractService.registerTicketValidation(
                    ticket.id,
                    ticketData.purchaseId || 'unknown-purchase',
                    userId,
                    eventId,
                    sectionName
                );
                ...
                return createApiResponse(HttpStatus.OK, {
                    ...
                    ticketData: {
                        ...
                        event: ticket.section.event ? {
                            address: ticket.section.event.address,
                            id: ticket.section.event.id,
                            date: ticket.section.event.start_date
                        } : null,
                        blockchain: {
     ...
     */
    async getTicketStatistics(userId: string): Promise<ApiResponse<any>> {
        try {
            const totalTickets = await this.ticketRepository.count();
            const availableTickets = await this.ticketRepository.count({
                where: { is_active: true }
            });
            const usedTickets = Math.max(totalTickets - availableTickets, 0);
