...
async function runStressTest() {
  console.log('🔥 INICIANDO PRUEBA DE FUEGO (GANADOR ASEGURADO) - 1050 ELECTORES 🔥');
    ...
    for (let i = 0; i < numBatches; i++) {
      ...
      const promesas = lote.map(async (elector) => {
        try {
          ...
          // C. Seleccionar candidatos (SESGADO AL 70% PARA EL PRIMER FRENTE)
          const selecciones = papeletaData.cargos.map((cargo: any) => {
            ...
            // SESGO: 70% de los votos van para el primer candidato (y por ende su frente)
            let randomCand;
            if (Math.random() < 0.70) {
              randomCand = todosLosCandidatos[0];
            } else {
              // El 30% restante se reparte entre los demás
              const otros = todosLosCandidatos.slice(1);
              if (otros.length > 0) {
                randomCand = otros[Math.floor(Math.random() * otros.length)];
              } else {
                randomCand = todosLosCandidatos[0];
              }
            }
